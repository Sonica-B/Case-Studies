...
NGROK_TEAMMATE
    ...
    echo "Started teammate's ngrok on port 5004"
    if [ -n "$TEAMMATE_NGROK_DOMAIN" ]; then
...
if [ -n "$TEAMMATE_NGROK_TOKEN" ] && curl -s http://localhost:5004/api/tunnels > /dev/null 2>&1; then
    echo ""
    echo "Teammate's Ngrok Services (Port 5004):"
